...
HTTPClient.prototype.open = function (connection, callback) {
    var url = connection.url || connection,
        ...
        isObject = 'object' === typeof data,
        isFunction = 'function' === typeof data,
        method = (connection.method || (data && 'POST' || 'GET')).toUpperCase(),
        headers = connection.headers || {},
        proxy = connection.proxy;

    if(isObject) {
        isBuffer || (data = querystring.stringify(data));
        if('GET' === method) {
            options.pathname =
                options.path = options.pathname + (~options.pathname.indexOf('?') && '&' || '?') + data;
        }
        else {
            headers['Content-Length'] || (headers['Content-Length'] = isBuffer ? data.length : Buffer.byteLength(data));
            // TODO: Add support for multipart/form-data boundary
            headers['Content-Type'] || (headers['Content-Type'] = isBuffer && 'multipart/form-data' || 'application/x-www-form-urlencoded');
        }
    }
    ...
    var contentType = '',
        ...
        request = new HTTPRequest(options)
            ...
            .on('data', onData)
            .once('end', function () {
                var result = Buffer.concat(answer, size);
