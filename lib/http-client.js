 ...
 *            {String|Object|Buffer} [connection.data] In case of:
 *                - String and Buffer sends as is with installing properly Content-Length header.
...
HTTPClient.prototype.open = function (connection, callback) {
    ...
    if('GET' === method) {
        ...
        if(!('Content-Length' in headers) && !('chunked' === headers['Transfer-Encoding'] || 'chunked' === headers['TE'])) {
            headers['Content-Length'] = Buffer.isBuffer(data)? data.length : Buffer.byteLength(
                'string' === typeof data? data : (data = querystring.stringify(data))
            );
        }
    }
